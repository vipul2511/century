...
renderItem = ({ item,index }) => 
{
  ...
  return(
    <View key={index}>
     <TouchableOpacity style={{flexDirection:'row',height:'auto',}} onPress={()=>{this.props.navigation.navigate('BPR',{dataItem:this.props.route.params.dataItem})}}>
  ...
  dataFetchStockItem=()=>{
    ...
    var EditProfileUrl = `http://demo.3ptec.com/dms-demo/mobile-json-data?logintoken=${JSON.stringify(this.state.token)}&sourcetype=AndroidSalesPersonApp&fileDataSource=salesorder-print&inputFieldsData={"selEntityId":${JSON.stringify(this.state.orgId)},"selEntityType":${JSON.stringify(this.state.type)},"salesOrderId":${orderID}}`
            ...
            if(responseData!=="No Data Found"){
              if(responseData!="lstDbReportChildEntities is null / empty"){
                ...
                if(Object.prototype.toString.call(responseData) === '[object Object]'){
                  ...
                  this.setState({tableData:arr,masterlist:arr});
