...
export default class BarChildGraph extends Component {
  constructor(props) {
    super(props);
    this.state = {
      text: "",
  ...
  salesReportData = (startdate, enddate) => {
    ...
    if(this.props.route.params.childVar){
      customerTy=JSON.stringify(this.props.route.params.DataItem.typecus);
      customerId=customerId=JSON.stringify("All");
    }
    else {
      customerId=customerTy=JSON.stringify("All");
    }
    ...
    var EditProfileUrl = `${BASE_URL}/dms-demo/DmsCommonReport?logintoken=${token}&sourcetype=AndroidSalesPersonApp&reportDataSource=sales-report&reportInputFieldsData={"selZoneId": ${zoneid},"childZoneFlag": "true","AllHierarchyFlag": "false","selCustomerType": ${entitytype},"selCustomerId": ${customerId},"selStartDateNum": ${startDate},"selEndDateNum": ${endDate},"fetchDataSource": "report-table-data","timeoffset": 330,"reportLevel": "Entity","month":${month},"year": "2021","entitytypename": ${type}}`
  ...
  searchFilterFunction = (text) => {
    ...
    if (text) {
      ...
      const newData = combineArray.filter(
        function (item) {
          const itemData = item.parentname
    ...
    return (
      <View key={index}>
        <TouchableOpacity style={{ flexDirection: 'row', height: 'auto', justifyContent: 'center', alignItems: 'center' }} >
          <View style={{ width: wp(100), alignSelf: 'center', flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ fontSize: 15, flexWrap: 'wrap', marginBottom: 10, textAlign: 'center' }}>{item.parentname}</Text>
          </View>
          {/* <View style={{width:wp(60),alignSelf:'center',flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
     <Text style={{fontSize:13,marginLeft:15,flexWrap:'wrap',marginBottom:10,textAlign:'center'}}>{item.monthMilliSec}</Text>
     </View> */}
          <TouchableOpacity style={{ width: wp(140), alignSelf: 'flex-end', flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }} onPress={() => { this.props.navigation.navigate('InnerBarGraph', { name: item.customername, data: this.props.route.params.data, start: this.props.route.params.start, enddate: this.props.route.params.enddate, month: this.props.route.params.month, zoneid: this.props.route.params.zoneid, token: this.props.route.params.token, customerid: item.customerid, dataMonth: language }) }}>
            <Text style={{ fontSize: 15, flexWrap: 'wrap', marginBottom: 10, textAlign: 'center', color: '#1976D2' }}>{item.customername}</Text>
          </TouchableOpacity>
          <View style={{ width: wp(100), alignSelf: 'flex-end', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        {!this.state.Nodata ? <View style={{ marginTop: 8, marginBottom: 5, marginLeft: 5, }}>
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <View style={{ width: wp(100), alignSelf: 'flex-start' }}>
              <Text style={{ fontWeight: 'bold', textAlign: 'center' }}>Parent Name</Text></View>
